...
export function DailySalesChart({ items }: DailySalesChartProps) {
  ...
  if (items.length === 0) {
    return <p className="py-8 text-center text-sm text-slate-500 dark:text-[#7F8D9A]">Sin datos.</p>;
  }
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.date} className="flex min-w-0 flex-1 flex-col items-center gap-2">
              <div className="flex h-40 w-full items-end rounded-md bg-slate-100 p-1 dark:bg-[#273342]/80">
                <div
              ...
              <div className="w-full text-center">
                <p className="text-[11px] font-semibold text-gray-950 dark:text-[#F3F7FA]">
                  {item.label}
                </p>
                <p className="mt-0.5 text-[11px] text-slate-500 dark:text-[#7F8D9A]">
                  {item.count}
                </p>
              </div>
            </div>
