...
export function printOfflineTicket(sale: OfflineCashSale) {
  ...
  popup.document.write(`<!doctype html>
    ...
    <table>${lines}</table>${sale.subtotalBeforeSurcharge ? `<p>Subtotal ${formatMoney(sale.subtotalBeforeSurcharge)}</p>` : ""}${sale.surcharge ? `<p>${escapeHtml(sale.surcharge.type === "PERCENTAGE" ? `Recargo ${sale.surcharge.value}%` : "Recargo")} ${formatMoney(sale.surcharge.amount)}</p>` : ""}<p class="total">TOTAL ${formatMoney(sale.total)}</p><p>Efectivo ${formatMoney(sale.cashReceived)}</p><p>Vuelto ${formatMoney(sale.changeAmount)}</p>
